...
#else
#ifdef GIZMONDO
#define VIDEO_FLAGS_INIT SDL_HWSURFACE|SDL_FULLSCREEN|SDL_HWPALETTE
#else
...
void black_screen_now(void)
{
	...
	SDL_Flip(prSDLScreen);
#else
	SDL_UpdateRect(prSDLScreen, 0, 0, 320, 256);
#endif
...
static void graphics_subinit (void)
{
	...
	else
	{
		...
		SDL_Flip(prSDLScreen);
#endif
#if !defined (DREAMCAST) && !defined (PSP) && !defined (GIZMONDO) && !defined (GP2X) && !defined(IPHONE)
