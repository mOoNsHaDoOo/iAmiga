...
struct uaedev_mount_info *alloc_mountinfo (void)
{
    ...
    info = (struct uaedev_mount_info *)xmalloc (sizeof *info);
...
static a_inode *lookup_child_aino_for_exnext (Unit *unit, a_inode *base, char *rel, uae_u32 *err)
{
    ...
    *err = 0;
    ...
    if (c == 0) {
	c = (a_inode *)xmalloc (sizeof (a_inode));
	if (c == 0) {
...
static int my_rmdir (Unit *unit, a_inode *root) {
	return rmdir (root->nname);
}
...
#ifdef USE_FAME_CORE
		/* FAMEC requires swabbed memory, word-padded.
...
static int get_new_device (char **devname, uaecptr *devname_amiga, int cdrom)
{
    ...
    if (cdrom) {
	snprintf (buffer, 80, "CD%d", current_cdrom);
	result = current_cdrom++;
    } else {
	snprintf (buffer, 80, "DH%d", current_deviceno);
...
void filesys_start_threads (void)
{
    ...
    current_mountinfo = dup_mountinfo (currprefs_mountinfo);
...
static uae_u32 filesys_dev_bootfilesys (void) //TrapContext *context)
{
    ...
    int type = is_hardfile (currprefs_mountinfo, unit_no);
...
static uae_u32 REGPARAM2 filesys_init_storeinfo (void) //TrapContext *context)
{
	int ret = -1;
...
static uae_u32 filesys_dev_storeinfo (void)
{
    ...
    /* WinUAE code */
    int sub_no = unit_no >> 16;
    unit_no = unit_no & 0xFFFF;
    if (sub_no)
    	return -2;
    /***************/
...
void filesys_install (void)
{
    ...
    calltrap (deftrap2 (filesys_init_storeinfo, 0/*TRAPFLAG_EXTRA_STACK*/, "filesys_init_storeinfo"));
    dw (RTS);
    /*******************/
