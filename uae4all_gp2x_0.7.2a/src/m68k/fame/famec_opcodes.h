...
OPCODE(0x0C3A)
{
    ...
    FETCH_BYTE(src);
    adr = GET_SWORD + (u32)((uintptr_t)(PC) - BasePC);
    PC++;
    READ_BYTE_F(adr, dst)
    res = dst - src;
    flag_N = flag_C = res;
...
OPCODE(0x6001)
{
	u32 newPC = (u32)((uintptr_t)(PC) - BasePC);
	s8 offs=Opcode;
	newPC += offs;
#ifdef OPCODES_M68000
    CHECK_BRANCH_EXCEPTION(newPC,10);
