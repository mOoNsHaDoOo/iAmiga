...
#else
static __inline__ uint16_t make_a_sr(void)
{
        uint16_t ret=0;
...
static __inline__ void dbg_cycle(uint32_t opcode)
{
...
static __inline__ uint32_t getmemsum(void)
{
        uint32_t *p=(uint32_t *)chipmemory;
        uint32_t i,ret=0;
