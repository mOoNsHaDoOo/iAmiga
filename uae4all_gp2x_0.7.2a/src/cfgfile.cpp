...
uaedev_mount_info *currprefs_mountinfo;

char uae4all_hard_dir[256];
char uae4all_hard_file[256];
...
#endif
	s2 = add_filesys_unit (currprefs_mountinfo, buf, s2, readonly, 0, 0, 0, 0);
	if (s2)
	    fprintf (stderr, "%s\n", s2);
    } else {
...
/*static*/ void parse_hardfile_spec (char *spec)
{
    ...
    *x3++ = '\0';
	...
	goto argh;
    *x4++ = '\0';
    x4 = add_filesys_unit (currprefs_mountinfo, 0, x4, 0, atoi (x0), atoi (x1), atoi (x2), atoi (x3));
    if (x4)
	fprintf (stderr, "%s\n", x4);
...
void init_mountinfo() {
    currprefs_mountinfo = alloc_mountinfo();
}
