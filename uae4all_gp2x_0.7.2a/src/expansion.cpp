...
static void allocate_expamem (void)
{
    ...
    if (allocated_fastmem != fastmem_size) {
    	if (fastmemory)
    		mapped_free (fastmemory);
    	fastmemory = 0;
        allocated_fastmem = fastmem_size;
...
void expamem_reset (void)
{
   ...
   if (nr_units (currprefs_mountinfo) == 0) {
      do_mount = 0;
   }
...
{
#if !( defined(PANDORA) || defined(ANDROIDSDL) || defined(IHPONE) )
    //z3fastmem = 0; // stoens: commented out when adding hd support
    //gfxmemory = 0;
#endif
    fastmemory = 0;
#if !( defined(PANDORA) || defined(ANDROIDSDL) || defined(IPHONE) )
...
void restore_fram (uae_u32 len, size_t filepos)
{
    ...
    //changed_prefs.fastmem_size = len; // stoens: commented out when adding hd support
}
